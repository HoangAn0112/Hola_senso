...
Le type de receuil de donnée est directement issu de la problématique posée. Voici une présentation des différents types de receuils rencontrés en sensométrie.
...
Cette méthodes et une méthode phare en méthode "libre". Elle consiste à positionner les stimulis les uns par rapport aux autres sur une feuilles blanche. Cette méthode permet de classer et d'ordonner les produits. Cette méthode se décline en deux variantes :
...
Création du jeu de données que l'on va analyser, composé des colonnes 4, 5, 6, 7, 9 et 11. Ce jeu de données est maintenant composé uniquement de variables quantitatives. Les colonnes 4 à 7 décrive la manière dont les enfant ont trié (nombre de hierarchies, nombre de classes, nom de critères cités, etc). On a 2 autres colonnes : Le temps passé avec l'enfants et l'âge de l'enfant.
```{r}
...
Remarque : 
Il est possible à l'aide d'une AFMH de prendre en compte une suite de partition emboitées, et par exemple prendre en comptre le processus congnitif du consommateur.
